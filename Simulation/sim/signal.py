...
def make_silence(sample_rate, duration):
	...
	return np.zeros(round(sample_rate * duration))

def make_chirp(sample_rate, center_freq, chirp_freq, duration):
	"Generates a chirp signal around a center frequency"
	min_freq = center_freq - chirp_freq
	time = np.linspace(0, duration, round(sample_rate * duration), False)
...
def correlate(signal_a, signal_b):
	...
	return np.correlate(signal_a, signal_b)
